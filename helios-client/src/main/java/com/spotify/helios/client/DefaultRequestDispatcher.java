  ...
  private HttpURLConnection connect(final URI uri, final String method, final byte[] entity,
    ...
    while (currentTimeMillis() < deadline) {
      ...
      for (final Endpoint endpoint : endpoints) {
        try {
          final InetAddress[] ips = InetAddress.getAllByName(endpoint.getHost());
          for (final InetAddress ip : ips) {
            final URI ipUri = new URI(
                endpoint.getScheme(), endpoint.getUserInfo(), ip.getHostAddress(), endpoint.getPort(),
                endpoint.getPath(), endpoint.getQuery(), endpoint.getFragment());
            ipEndpoints.add(ipUri);
            ipToHostnameUris.put(ipUri, endpoint);
          }
        } catch (UnknownHostException e) {
          log.warn("Unable to resolve hostname {} into IP address: {}", endpoint.getHost(), e);
        }
      }
