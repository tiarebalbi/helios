...
public class HeliosClient implements AutoCloseable {
  ...
  private ListenableFuture<Response> request(final URI uri, final String method,
    ...
    return executorService.submit(new Callable<Response>() {
      ...
      public Response call() throws Exception {
        final HttpResponse response = connect(uri, method, entityBytes, headers);
        final int status = response.getStatusLine().getStatusCode();
        final InputStream rawStream = response.getEntity().getContent();
   ...
   */
  private HttpResponse connect(final URI uri, final String method, final byte[] entity,
  ...
  private HttpResponse connect0(final URI ipUri, final String method, final byte[] entity,
                                final Map<String, List<String>> headers,
