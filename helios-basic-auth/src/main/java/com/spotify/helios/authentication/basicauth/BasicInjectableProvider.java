...
/**
 * A Jersey provider for basic access authentication.
 *
 * @param <T> the principal type.
 */
public class BasicInjectableProvider<T> implements InjectableProvider<Auth, Parameter> {
  ...
  private static class BasicAuthInjectable<T> extends AbstractHttpContextInjectable<T> {
    ...
    private BasicAuthInjectable(final Authenticator<String, T> authenticator,
                                boolean required) {
      ...
      if (required) {
        throw new WebApplicationException(
            Response.status(Response.Status.UNAUTHORIZED)
                ...
                .header(HttpHeaders.WWW_AUTHENTICATE, "Basic realm=\"PUT SOMETHING HERE?\"")
   ...
   */
  public BasicInjectableProvider(final Authenticator<String, T> authenticator) {
    this.authenticator = authenticator;
  }
